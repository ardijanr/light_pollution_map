...
use crate::common::{generate_gradient, get_cache_from_file, write_cache_to_file, ParArray, length, length_square_i32};
...
pub fn version_4_crawler(){
    ...
    if let Some(cache_file) = get_cache_from_file(){
        ...
        for light in 0..20_000{
            for dist in 1..MAX_DIST{
                ...
                let lp = garstang::garstang_1989_calc( (light as f64)*683.*750.*750., (dist as f64)/10.,0.,0., obs_dir)*1000.;
                if light%100==0{
                ...
                if lp<1.{
                    if light%100==0{
                        println!("source light: {}, SETTING MAX INDEX: {} KM, lp_scaled: {}",light,dist/10,lp);
                    }
            ...
            loop {
                ...
                for wx in 0..dim {
                    ...
                    for _y in -dim_local..dim_local as i32{
                        let actual_y = y-_y;
                        if actual_y<0 || actual_y>=dim { continue; }
                    ...
                    let mut calc_offset =|| {
                        let p1 = point!(x: offset_lat +(x as f64*PIXEL_DIM), y: offset_lon + (y as f64*PIXEL_DIM));
                        let p2 = point!(x: offset_lat +((x+1) as f64 * PIXEL_DIM), y: offset_lon+ ((y) as f64*PIXEL_DIM));
                        delta.0 = (p1.vincenty_distance(&p2).unwrap()/100.) as f32;

                        let p1 = point!(x: offset_lat +(x as f64*PIXEL_DIM), y: offset_lon + (y as f64*PIXEL_DIM));
                        let p2 = point!(x: offset_lat +((x) as f64 * PIXEL_DIM), y: offset_lon+ ((y+1) as f64*PIXEL_DIM));
                    ...
                    for index in 0..relevant_pixels.len(){
                        ...
                        let dist = length((delta.0*rel_dist.0 as f32),(delta.1*rel_dist.1 as f32)) as usize;
                        if dist>=3000{
                            continue;
                        }
    ...
    for y in 0..dim as usize{
        for x in 0..dim as usize{
            ...
            generated_image.get_pixel_mut(x as u32, y as u32).0 = [color[0],color[1],color[2], sum_color as u8];
        }
    }
